    ...
    <div class="register">
      <h3>注册新用户
        <span class="go">我有账号，去 
          <a href="login.html" target="_blank" @click.prevent="$router.push('/login')">登陆</a>
        </span>
      </h3>
      <div class="content">
        <label>手机号:</label>
    ...
    methods:{
     async register(){//注册用户
        ...
        if(phone&&password&&password2&&code&&password===password2){
          try{
            await this.$store.dispatch('getRegister',{phone,password,password2,code})
            alert('注册成功去往登录页面')
          }catch(error){
       ...
       async getCode(){
         try{
           ...
           this.code = this.$store.state.home.code
         }catch(error){
           alert('shibai')
         }
