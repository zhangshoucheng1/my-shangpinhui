    ...
    <div class="cart-main">
      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="(cart, index) in cartInfoList" :key="index">
          <li class="cart-list-con1" @click="updateOneCheck(cart)">
          ...
          <li class="cart-list-con5">
            ...
            @click.prevent="changCartNum(cart,-1,true)">-</a>
            <input autocomplete="off" type="text"
             :value="cart.skuNum" minnum="1" class="itxt"  
             @change="changCartNum(cart,$event.target.value*1,false)">
...
import { mapGetters, mapState } from 'vuex'
  ...
  export default {
    ...
    methods:{
     ...
     async changCartNum(cart,disNum,flag){//点击+或—
     ...
     async updateOneCheck(cart){
       try{
         await this.$store.dispatch('getUpdateCartOneIsCheck',
        {skuId:cart.skuId,isChecked:cart.isChecked? 0 : 1})
        // alert('修改成功')
         this.getShopCart()
       }catch(error){
         alert('shibai')
     ...
     async deleteOne(cart){//删除购物车列表单个数据
        try{
          await this.$store.dispatch('getDeleteOneCart',cart.skuId)
     ...
     async deleteAll(){//删除购物车列表已选中数据
        try{
         await this.$store.dispatch('deleteAllCart')
         this.getShopCart()
      }catch(error){
        alert('删除购物车列表已选中数据shibai')
      }
    ...
    computed:{
      ...mapState({
        shopCart:state=>state.detail.shopCart
      }),
      ...mapGetters(['cartInfo']),
      cartInfoList(){
